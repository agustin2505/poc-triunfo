...
export default function MetricasPage() {
  ...
  const pieData = Object.entries(routingDistribution).map(([key, value]) => ({
    name: routingLabels[key],
    value,
    color: routingColors[key],
  }))

  // Prepare confidence distribution data
  const confidenceData = Object.entries(confidenceDistribution).map(([range, values]) => ({
    range,
    DocAI: values.docai,
    Tesseract: values.tesseract,
    ...
    <div className="min-h-[calc(100vh-4rem)] p-6">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6">
          ...
          <p className="text-slate-500">Métricas de procesamiento del día actual</p>
        </div>
                    ...
                    <TableRow key={agent.name}>
                      ...
                      <TableCell className="text-right">
                        <span className={cn(
                          ...
                          agent.successRate >= 0.90 ? "text-yellow-600" : "text-red-600"
                        )}>
                          {(agent.successRate * 100).toFixed(0)}%
                        </span>
                      </TableCell>
                      <TableCell className="text-right text-slate-500">
                        {(agent.timeoutRate * 100).toFixed(1)}%
                      </TableCell>
                      <TableCell className="text-right">{agent.avgDuration}s</TableCell>
                      <TableCell className="text-right">
                        <span className={cn(
                          "font-medium",
                          agent.avgConfidence >= 0.88 ? "text-emerald-600" : 
                          agent.avgConfidence >= 0.70 ? "text-yellow-600" : "text-red-600"
                        )}>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6">
          <h3 className="mb-4 font-semibold text-slate-900">Distribución de Confidence por Agente</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
