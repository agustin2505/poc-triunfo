"""Agente Gemini 2.5 Flash (Vertex AI) — Spec-15."""
...
_logger = setup_logging("triunfo.agents.gemini_flash_lite")


class GeminiFlashLiteAgent(BaseAgent):
    """Agente de máxima velocidad — gemini-2.5-flash.

    Primer resultado disponible para el orquestador.
    Referencia de latencia para la métrica P95 < 5s del MVP.
    """
    ...
    def _init_model(self) -> None:
        ...
        _logger.info(f"GeminiFlashLiteAgent inicializado: {model_id} @ {self._location}")
    ...
    def _extract(
        ...
        **kwargs,
        ...
        if not image_bytes:
            raise ValueError("GeminiFlashLiteAgent requiere image_bytes")
        ...
        _logger.debug(
            f"[{document_id[:8]}] gemini-flash-lite extrajo "
    ...
    def _call_with_retry(self, parts, gen_cfg, max_retries: int = 1):
        """1 reintento en errores 429/500/503. No reintentar en timeout."""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                retryable = (
                    (str(code) in {"RESOURCE_EXHAUSTED", "UNAVAILABLE", "INTERNAL"} if code else False)
                    or (http_code in {429, 500, 503})
                )
                if retryable and attempt < max_retries:
                    _logger.warning(
                        f"gemini-flash-lite error retryable (attempt {attempt+1}): {exc}. Reintentando..."
                    )
                    time.sleep(2)
                    continue
                raise
