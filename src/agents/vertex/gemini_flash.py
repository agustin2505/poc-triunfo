"""Agente Gemini 2.0 Flash (Vertex AI) — Spec-13."""
...
_logger = setup_logging("triunfo.agents.gemini_flash")

_RETRYABLE_CODES = {429, 500, 503}


class GeminiFlashAgent(BaseAgent):
    """Agente de extracción visual equilibrado — gemini-2.0-flash.

    Corre en paralelo con GeminiProAgent y GeminiFlashLiteAgent dentro
    del VertexOrchestrator (Spec-16).
    """
    ...
    # ------------------------------------------------------------------
    # Inicialización lazy (evita crash si GOOGLE_CLOUD_PROJECT no está set)
    # ------------------------------------------------------------------
    def _init_model(self) -> None:
        ...
        if not project:
            raise RuntimeError(
                ...
                "Necesaria para usar Vertex AI."
            )

        self._location = os.getenv("VERTEX_AI_LOCATION", "us-central1")
        model_id = os.getenv("GEMINI_FLASH_MODEL", "gemini-2.0-flash")

        vertexai.init(project=project, location=self._location)
        self._model = GenerativeModel(model_id)
        self._model_id = model_id
        _logger.info(f"GeminiFlashAgent inicializado: {model_id} @ {self._location}")
    ...
    def _extract(
        ...
        **kwargs,
        ...
        if not image_bytes:
            raise ValueError("GeminiFlashAgent requiere image_bytes")
        ...
        _logger.debug(
            f"[{document_id[:8]}] gemini-flash extrajo "
    ...
    def _call_with_retry(self, parts, gen_cfg, max_retries: int = 1):
        """Llama al modelo con 1 reintento en errores 429/500/503."""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                retryable = (
                    (code is not None and str(code) in {"RESOURCE_EXHAUSTED", "UNAVAILABLE", "INTERNAL"})
                    or (http_code in _RETRYABLE_CODES)
                )
                if retryable and attempt < max_retries:
                    _logger.warning(
                        f"gemini-flash error retryable (attempt {attempt+1}): {exc}. Reintentando..."
                    )
